...
function afterResize() {
    ...
    if (!pixelValues || pixelValues.length !== totalPixels) {
        pixelValues = new Uint8Array(totalPixels);
    }
...
function render() {
    ...
    for (i = 0; i < imageDataPaint.data.length; i += 4) {
        ...
        j = pixelValues[idx] = pixelValues[((idx - 1) + totalPixels) % totalPixels] * 0.3 +
                                pixelValues[((idx - width) + totalPixels) % totalPixels] * 0.3 + j * 0.4;
        idx += 1;
        ...
        //imageDataPaint.data[i] = imageDataPaint.data[i + 1] = imageDataPaint.data[i + 2] = Math.round(j / 51) * 51;
    }
